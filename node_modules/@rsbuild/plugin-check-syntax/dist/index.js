...
import * as __WEBPACK_EXTERNAL_MODULE_browserslist_to_es_version_8145aa74__ from "browserslist-to-es-version";
...
function displayCodePointer(code, pos) {
    ...
    const arrowLine = __WEBPACK_EXTERNAL_MODULE_picocolors__["default"].yellow('^'.padStart(arrowPos + 11, ' '));
    return `${subCode}\n${arrowLine}`;
}
...
function makeCodeFrame(lines, highlightIndex) {
    ...
    for(let i = startLine; i < endLine; i++)if (i === highlightIndex) {
        ...
        ret.push(__WEBPACK_EXTERNAL_MODULE_picocolors__["default"].yellow(`${lineNumber} | ${lines[i]}`));
    } else {
        const lineNumber = ` ${i + 1}`.padStart(6, ' ');
        ret.push(__WEBPACK_EXTERNAL_MODULE_picocolors__["default"].gray(`${lineNumber} | ${lines[i]}`));
    }
    return `\n${ret.join('\n')}`;
}
async function tryGenerateErrorFromSourceMap({ err, filepath, rootPath }) {
    const sourceMapPath = `${filepath}.map`;
    if (!__WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__["default"].existsSync(sourceMapPath)) return null;
    try {
        var _JSON_parse_sourcesContent, _JSON_parse;
        const sourcemap = await __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__["default"].promises.readFile(sourceMapPath, 'utf-8');
        const consumer = await new __WEBPACK_EXTERNAL_MODULE_source_map_c2f0c629__.SourceMapConsumer(sourcemap);
...
async function generateHtmlScripts(filepath) {
    const html = await __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__["default"].promises.readFile(filepath, 'utf-8');
...
class CheckSyntax {
    async check(filepath, code) {
        ...
        if (JS_REGEX.test(filepath)) {
            const jsScript = await __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__["default"].promises.readFile(filepath, 'utf-8');
            await this.tryParse(filepath, jsScript);
        }
    }
    async tryParse(filepath, code) {
        try {
            (0, __WEBPACK_EXTERNAL_MODULE_acorn__.parse)(code, {
    ...
    constructor(options){
        ...
        this.ecmaVersion = ecmaVersion || (0, __WEBPACK_EXTERNAL_MODULE_browserslist_to_es_version_8145aa74__.browserslistToESVersion)(targets);
...
function printErrors(errors, ecmaVersion, excludeErrorLogs) {
    ...
    const expectedVersion = __WEBPACK_EXTERNAL_MODULE_picocolors__["default"].yellow(`ecmaVersion <= ${ecmaVersion}`);
    logger.error(`[@rsbuild/plugin-check-syntax] Find some syntax that does not match "${expectedVersion}":\n`);
    errs.forEach((err, index)=>{
        console.log(__WEBPACK_EXTERNAL_MODULE_picocolors__["default"].bold(__WEBPACK_EXTERNAL_MODULE_picocolors__["default"].red(`  ERROR ${index + 1}`)));
...
function printMain(error, longest, excludeErrorLogs) {
    ...
    for (const [key, content] of Object.entries(error)){
        ...
        const title = __WEBPACK_EXTERNAL_MODULE_picocolors__["default"].magenta(`${fillWhiteSpace(`${key}:`, longest + 1)}`);
    ...
    apply(compiler) {
        ...
        compiler.hooks.afterEmit.tapPromise(CheckSyntaxRspackPlugin.name, async (compilation)=>{
            ...
            const emittedAssets = compilation.getAssets().filter((a)=>a.source).map((a)=>{
                ...
                const file = (0, __WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__.resolve)(outputPath, resourcePath);
