    ...
    if (container.current &&
        ...
        if (count > 0) {
            ...
            const offset = selOffset > viewport
                ? Math.min(viewport - selOffset - (selected < count - 1 ? itemWidth / 2 : 0), paneWidth - viewport)
                : 0;
...
export function CommitTimeline(props) {
    ...
    }, [pane, container, paneContainerRef, inner, items.length, selectedItemIndex], true);
    ...
    return (<div class="commit-timeline" ref={container}>
			<div class="commit-timeline-inner" ref={inner}>
				...
				<button disabled={items.length <= 1} onClick={onPrev} class="commit-time-line-nav-btn" data-testid="prev-commit">
					<Icon icon="arrow-back"/>
				</button>
				<div class="commit-timeline-item-container" ref={paneContainerRef} style={`width: ${viewportWidth}px`}>
					<div class="commit-timeline-items" ref={pane} style={`width: ${paneWidth}px; transform: translate3d(${offset}px, 0, 0);`}>
						{items.map(x => {
            return (<CommitItem key={x.index} onClick={() => onChange(x.index)} selected={x.index === selected} percent={x.percent}/>);
        })}
					</div>
				</div>
				<button disabled={items.length <= 1} onClick={onNext} class="commit-time-line-nav-btn" data-testid="next-commit">
					<Icon icon="arrow-forward"/>
				</button>
			</div>
		</div>);
}
export function CommitItem(props) {
    const { percent } = props;
    const top = Math.max(20, Math.min(86, 100 - percent));
    const color = getGradient(100, percent);
    return (<div data-testid="commit-item" class="commit-timeline-item" data-selected={props.selected} data-weight={color} onClick={props.onClick}>
