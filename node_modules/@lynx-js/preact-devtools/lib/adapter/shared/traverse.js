...
import { getOrCreateVNodeId, getVNodeById, getVNodeId, hasVNodeId, updateVNodeId, } from "./idMapper";
...
// 	}
// }
export function getFilteredChildren(vnode, filters, config, helpers) {
...
export function shouldFilter(vnode, filters, config, bindings) {
...
function mount(ids, commit, owners, vnode, ownerId, ancestorId, filters, domCache, config, profiler, hocs, bindings, timingsByVNode, renderReasonPre) {
    ...
    if (root || !skip) {
        ...
        ancestorId, ownerId, getStringId(commit.strings, name), vnode.key ? getStringId(commit.strings, vnode.key) : 0, 
        ...
        if (profiler.isProfiling && profiler.captureRenderReasons) {
            commit.operations.push(MsgTypes.RENDER_REASON, id, 1 /* RenderReason.MOUNT */, 0);
        }
...
function update(ids, commit, owners, vnode, ownerId, ancestorId, filters, domCache, config, profiler, hocs, bindings, timingsByVNode, renderReasonPre) {
    ...
    if (profiler.isProfiling && profiler.captureRenderReasons) {
        const reason = renderReasonPre !== null
            ...
            : bindings.getRenderReasonPost(ids, bindings, timingsByVNode, oldVNode, vnode);
        if (reason !== null) {
...
export function createCommit(ids, roots, owners, vnode, filters, domCache, config, profiler, helpers, timingsByVNode, renderReasonPre) {
    const commit = {
        operations: [],
        rootId: -1,
        strings: new Map(),
